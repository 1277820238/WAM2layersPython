...
years = np.arange(2012, 2017)  # fill in the years
yearpart = np.arange(0, 30)
...
lake_mask_1 = np.array(
    [9, 9, 9, 12, 12, 21, 21, 22, 22, 23, 24, 25, 23, 23, 25, 25, 53, 54, 61, 23, 24, 23, 24, 25, 27, 22, 23, 24, 25,
     26, 27, 28, 22, 25, 26, 27, 28, 23, 23, 12, 18])
lake_mask_2 = np.array(
    [120 + 19, 120 + 40, 120 + 41, 120 + 43, 120 + 44, 120 + 61, 120 + 62, 120 + 62, 120 + 63, 120 + 62, 120 + 62,
     120 + 62, 120 + 65, 120 + 66, 120 + 65, 120 + 66, 142 - 120, 142 - 120, 143 - 120, 152 - 120, 152 - 120, 153 - 120,
     153 - 120, 153 - 120, 153 - 120, 154 - 120, 154 - 120, 154 - 120, 154 - 120, 154 - 120, 154 - 120, 154 - 120,
     155 - 120, 155 - 120, 155 - 120, 155 - 120, 155 - 120, 159 - 120, 160 - 120, 144 - 120, 120 + 55])
# lake_mask_1 = np.array([32, 33, 34, 35])
# lake_mask_2 = np.array([153-120, 154-120, 155-120, 156-120])
...
output_folder = r'F:\WAM2\WAM2layersPython\ERA-Interim\DATA\output'  # 这里要用\ 表示back slash
# END OF INPUT
...
def data_path(y, a):
    ...
    save_path = os.path.join(output_folder,
                             'P_track_continental_full' + str(years[0]) + '-' + str(years[-1]) + '-timetracking' + str(
                                 timetracking) + '.mat')

    save_path_daily = os.path.join(output_folder, 'P_track_continental_daily_full' + str(y) + '-timetracking' + str(
        timetracking) + '.mat')
...
E_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
P_track_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
P_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
Sa_track_down_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
Sa_track_top_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
W_down_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
W_top_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
north_loss_per_year_per_month = np.zeros((len(years), 1, 1, len(longitude)))
south_loss_per_year_per_month = np.zeros((len(years), 1, 1, len(longitude)))
down_to_top_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
top_to_down_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
water_lost_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))

if timetracking == 1:
    Sa_time_down_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
    Sa_time_top_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
    P_time_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
...
for i in range(len(years)):
    ...
    # E_per_day = np.zeros((91, len(latitude), len(longitude)))
    # P_track_per_day = np.zeros((91, len(latitude), len(longitude)))
    # P_per_day = np.zeros((91, len(latitude), len(longitude)))
    # Sa_track_down_per_day = np.zeros((91, len(latitude), len(longitude)))
    # Sa_track_top_per_day = np.zeros((91, len(latitude), len(longitude)))
    # W_down_per_day = np.zeros((91, len(latitude), len(longitude)))
    # W_top_per_day = np.zeros((91, len(latitude), len(longitude)))
    # north_loss_per_day = np.zeros((91, 1, len(longitude)))
    # south_loss_per_day = np.zeros((91, 1, len(longitude)))
    # down_to_top_per_day = np.zeros((91, len(latitude), len(longitude)))
    # top_to_down_per_day = np.zeros((91, len(latitude), len(longitude)))
    # water_lost_per_day = np.zeros((91, len(latitude), len(longitude)))
    # if timetracking == 1:
    #     Sa_time_down_per_day = np.zeros((91, len(latitude), len(longitude)))
    #     Sa_time_top_per_day = np.zeros((91, len(latitude), len(longitude)))
    #     P_time_per_day = np.zeros((91, len(latitude), len(longitude)))

    E_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    P_track_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    P_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    Sa_track_down_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    Sa_track_top_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    W_down_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    W_top_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    north_loss_per_day = np.zeros((31 + ly, 1, len(longitude)))
    south_loss_per_day = np.zeros((31 + ly, 1, len(longitude)))
    down_to_top_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    top_to_down_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    water_lost_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    if timetracking == 1:
        Sa_time_down_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
        Sa_time_top_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
        P_time_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))

    # yearend = 366 if y % 4 == 0 else 364
    ...
    for m in range(1):
        ...
        Sa_track_down_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.mean(Sa_track_down_per_day[days, :, :], axis=0)))
        Sa_track_top_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.mean(Sa_track_top_per_day[days, :, :], axis=0)))
        ...
        north_loss_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.sum(north_loss_per_day[days, :, :], axis=0)))
        south_loss_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.sum(south_loss_per_day[days, :, :], axis=0)))
        down_to_top_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.sum(down_to_top_per_day[days, :, :], axis=0)))
        top_to_down_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.sum(top_to_down_per_day[days, :, :], axis=0)))
        water_lost_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.sum(water_lost_per_day[days, :, :], axis=0)))

        if timetracking == 1:
            Sa_time_down_per_year_per_month[y - startyear, m, :, :] = (
                    np.squeeze(np.mean(Sa_time_down_per_day[days, :, :]
            ...
            Sa_time_top_per_year_per_month[y - startyear, m, :, :] = (np.squeeze(np.mean(Sa_time_top_per_day[days, :, :]
                                                                                         * Sa_track_top_per_day[days, :,
                                                                                           :], axis=0))
                                                                      / np.squeeze(
                        Sa_track_top_per_year_per_month[y - startyear, m, :, :]))
            P_time_per_year_per_month[y - startyear, m, :, :] = (np.squeeze(np.sum(P_time_per_day[days, :, :]
                                                                                   * P_track_per_day[days, :, :],
                                                                                   axis=0))
                                                                 / np.squeeze(
                        P_track_per_year_per_month[y - startyear, m, :, :]))
...
sio.savemat(datapath[3],
            {'E_per_year_per_month': E_per_year_per_month, 'P_track_per_year_per_month': P_track_per_year_per_month,
