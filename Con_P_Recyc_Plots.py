...
def data_path(years, timetracking):
    load_data_pypm = os.path.join(output_folder, 'P_track_continental_full' + str(years[0]) + '-' + str(
        years[-1]) + '-timetracking' + str(timetracking) + '.mat')
...
vmax = 1.0
cmap = LinearSegmentedColormap.from_list('mycmap', [(0 / vmax, 'purple'),
                                                    (0.01 / vmax, 'blue'),
                                                    (0.2 / vmax, 'cyan'),
                                                    (0.3 / vmax, 'yellow'),
                                                    (0.4 / vmax, 'orange'),
...
d = nc.Dataset('2012-2016-1-P.nc', 'w')
d.createDimension('longitude', 44)
d.createDimension('latitude', 17)

d.createVariable('longitude', 'f', ('longitude'))
d.createVariable('latitude', 'f', ('latitude'))
d.createVariable('rho_c_landtotal', 'f', ('latitude', 'longitude'))


y_label = pd.DataFrame(y)

d.variables['longitude'][:] = x[0]
d.variables['latitude'][:] = y_label[0]
d.variables['rho_c_landtotal'][:] = rho_c_landtotal


d.close

shownctest = nc.Dataset(r'2012-2016-1-P.nc', 'r')
print(shownctest.variables['rho_c_landtotal'][:])
##########################################################################################################################


map.colorbar(cs, location='right', pad="5%", ticks=clevs, label='(-)')

windplot = map.quiver(x, y, Fa_E_total_m2a_part, Fa_N_total_m2a_part, color='Black', scale=39999999, latlon=True,
                      width=0.002, headaxislength=5, headwidth=2)
# adjust scale till arrows are visible

plt.title('Continental precipitation recycling ratio ' r'$\rho_c$', fontsize=16)
plt.show()

# # %% ################### GRIDCELL PLOT
#
# plt.figure(
#     1)  # nice projections are 'cea', 'cyl', 'eck4' and 'robin', however only 'cea' and 'cyl' allows for not showing the entire world
# map = Basemap(projection='cyl', lat_0=0, lon_0=0,
...
#
# windplot = map.quiver(x, y, Fa_E_total_m2a_part, Fa_N_total_m2a_part, color='	black', scale=199999999, latlon=True,
#                       width=0.002, headaxislength=5, headwidth=2)
# # adjust scale till arrows are visible
#
...
if timetracking == 1:
    ...
    Sa_time_down_average = np.squeeze(
        np.mean(np.mean(Sa_time_down_per_year_per_month, axis=0), axis=0)) / 86400  # d quick assesment
    Sa_time_top_average = np.squeeze(
        np.mean(np.mean(Sa_time_top_per_year_per_month, axis=0), axis=0)) / 86400  # d quick assesment
    Sa_time_average = ((np.squeeze(
        np.mean(np.mean(Sa_time_down_per_year_per_month * Sa_track_down_per_year_per_month, axis=0), axis=0))
                        + np.squeeze(
                np.mean(np.mean(Sa_time_top_per_year_per_month * Sa_track_top_per_year_per_month, axis=0), axis=0))
                        ) / Sa_track_average / 86400)  # d

    Sa_track_down_average = np.squeeze(np.mean(np.mean(Sa_track_down_per_year_per_month, axis=0), axis=0))  # m3
    Sa_track_top_average = np.squeeze(np.mean(np.mean(Sa_track_top_per_year_per_month, axis=0), axis=0))  # m3

    age_down_track_water_global_mean = (
            np.sum(np.sum(Sa_time_down_average[1:-1, :] * Sa_track_down_average[1:-1, :], axis=0), axis=0)
            / np.sum(np.sum(Sa_track_down_average[1:-1, :], axis=0), axis=0))  # d
    age_top_track_water_global_mean = (
    ...
    age_track_water_global_mean_atmos_weight2 = (
            (np.sum(np.sum(Sa_time_down_average[1:-1, :] * Sa_track_down_average[1:-1, :], axis=0), axis=0)
             + np.sum(np.sum(Sa_time_top_average[1:-1, :] * Sa_track_top_average[1:-1, :], axis=0), axis=0))
            / (np.sum(np.sum(Sa_track_down_average[1:-1, :], axis=0), axis=0) + np.sum(
        np.sum(Sa_track_top_average[1:-1, :], axis=0), axis=0)))  # d

    age_track_water_global_mean_atmos_weight = ((np.sum(
        Sa_time_down_per_year_per_month[:, :, 1:-1, :] * Sa_track_down_per_year_per_month[:, :, 1:-1, :])
                                                 + np.sum(
                Sa_time_top_per_year_per_month[:, :, 1:-1, :] * Sa_track_top_per_year_per_month[:, :, 1:-1, :]))
                                                / (np.sum(Sa_track_down_per_year_per_month[:, :, 1:-1, :]) + np.sum(
                Sa_track_top_per_year_per_month[:, :, 1:-1, :]))) / 86400  # d

    # weighted with precipitation 
    age_track_water_global_mean_precip_weight = (
            np.sum(P_time_per_year_per_month[:, :, 1:-1, :] * P_track_per_year_per_month[:, :, 1:-1, :])
