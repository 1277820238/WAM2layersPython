...
years = np.arange(2017, 2018)  # fill in the years
...
interdata_folder = r'F:\WAM2\WAM2layersPython\ERA-Interim\DATA\interdata'  # must be an existing folder, existence is not checked
...
def data_path_ea(years, yearpart):
    save_empty_arrays_ly_track = os.path.join(sub_interdata_folder, str(years[0] - 1) + '-' + str(30) + 'Sa_track.mat')
    save_empty_arrays_ly_time = os.path.join(sub_interdata_folder, str(years[0] - 1) + '-' + str(30) + 'Sa_time.mat')

    save_empty_arrays_track = os.path.join(sub_interdata_folder,
                                           str(years[0]) + '-' + str(yearpart[0] - 1) + 'Sa_track.mat')
    save_empty_arrays_time = os.path.join(sub_interdata_folder,
                                          str(years[0]) + '-' + str(yearpart[0] - 1) + 'Sa_time.mat')
...
def get_Sa_track_forward(latitude, longitude, count_time, divt, Kvf, Region, Fa_E_top, Fa_N_top, Fa_E_down, Fa_N_down,
    ...
    for t in range(np.int(count_time * divt)):
        ...
        if isglobal == 1:
            Sa_track_E_down[0, :, -1] = Sa_track_down[t, :,
                                        0]  # Atmospheric tracked storage of the cell to the east [m3]
        Sa_track_W_down[0, :, 1:] = Sa_track_down[t, :, :-1]  # Atmospheric tracked storage of the cell to the west [m3]
        if isglobal == 1:
            Sa_track_W_down[0, :, 0] = Sa_track_down[t, :,
                                       -1]  # Atmospheric tracked storage of the cell to the west [m3]
        Sa_track_N_down[0, 1:, :] = Sa_track_down[t, :-1,
                                    :]  # Atmospheric tracked storage of the cell to the north [m3]
        Sa_track_S_down[0, :-1, :] = Sa_track_down[t, 1:,
        ...
        down_to_top[t, :, :] = np.reshape(np.maximum(0, np.reshape(Sa_track_after_Fa_P_E_down,
                                                                   (np.size(Sa_track_after_Fa_P_E_down))) - np.reshape(
            W_down[t + 1, :, :],
            (np.size(W_down[t + 1, :, :])))), (len(latitude), len(longitude)))
        top_to_down[t, :, :] = np.reshape(np.maximum(0, np.reshape(Sa_track_after_Fa_P_E_top,
                                                                   (np.size(Sa_track_after_Fa_P_E_top))) - np.reshape(
            W_top[t + 1, :, :],
        ...
        water_lost_down[t, :, :] = np.reshape(np.maximum(0, np.reshape(Sa_track_after_all_down,
                                                                       (np.size(Sa_track_after_all_down))) - np.reshape(
            W_down[t + 1, :, :],
            (np.size(W_down[t + 1, :, :])))), (len(latitude), len(longitude)))
        water_lost_top[t, :, :] = np.reshape(np.maximum(0, np.reshape(Sa_track_after_all_top,
                                                                      (np.size(Sa_track_after_all_top))) - np.reshape(
            W_top[t + 1, :, :],
            (np.size(W_top[t + 1, :, :])))), (len(latitude), len(longitude)))
        water_lost[t, :, :] = water_lost_down[t, :, :] + water_lost_top[t, :, :]

        # down: determine Sa_region of this next timestep 100% stable
        Sa_track_down[t + 1, 1:-1, :] = np.reshape(np.maximum(0, np.minimum(
            np.reshape(W_down[t + 1, 1:-1, :], np.size(W_down[t + 1, 1:-1, :])),
            np.reshape(Sa_track_after_all_down[0, 1:-1, :],
        ...
        Sa_track_top[t + 1, 1:-1, :] = np.reshape(np.maximum(0, np.minimum(
            np.reshape(W_top[t + 1, 1:-1, :], np.size(W_top[t + 1, 1:-1, :])),
            np.reshape(Sa_track_after_all_top[0, 1:-1, :],
...
def get_Sa_track_forward_TIME(latitude, longitude, count_time, divt, timestep, Kvf, Region, Fa_E_top, Fa_N_top,
                              Fa_E_down, Fa_N_down, Fa_Vert, E, P,
                              W_top, W_down, Sa_track_top_last, Sa_track_down_last, Sa_time_top_last,
                              Sa_time_down_last):
    # make E_region matrix
    ...
    for t in range(np.int(count_time * divt)):
        ...
        Sa_time_after_Fa_down[0, 1:-1, :] = ((Sa_track_down[t, 1:-1, :] * (ti + Sa_time_down[t, 1:-1, :])
                                              - Fa_E_down_WE[t, 1:-1, :] * (ti + Sa_time_down[t, 1:-1, :]) * (
                                                      Sa_track_down[t, 1:-1, :] / W_down[t, 1:-1, :])
                                              + Fa_E_down_EW[t, 1:-1, :] * (ti + Sa_time_E_down[0, 1:-1, :]) * (
                                                      Sa_track_E_down[0, 1:-1, :] / Sa_E_down[0, 1:-1, :])
                                              + Fa_W_down_WE[t, 1:-1, :] * (ti + Sa_time_W_down[0, 1:-1, :]) * (
                                                      Sa_track_W_down[0, 1:-1, :] / Sa_W_down[0, 1:-1, :])
                                              - Fa_W_down_EW[t, 1:-1, :] * (ti + Sa_time_down[t, 1:-1, :]) * (
                                                      Sa_track_down[t, 1:-1, :] / W_down[t, 1:-1, :])
                                              - Fa_N_down_SN[t, 1:-1, :] * (ti + Sa_time_down[t, 1:-1, :]) * (
                                                      Sa_track_down[t, 1:-1, :] / W_down[t, 1:-1, :])
                                              + Fa_N_down_NS[t, 1:-1, :] * (ti + Sa_time_N_down[0, 1:-1, :]) * (
                                                      Sa_track_N_down[0, 1:-1, :] / Sa_N_down[0, 1:-1, :])
                                              + Fa_S_down_SN[t, 1:-1, :] * (ti + Sa_time_S_down[0, 1:-1, :]) * (
                                                      Sa_track_S_down[0, 1:-1, :] / Sa_S_down[0, 1:-1, :])
                                              - Fa_S_down_NS[t, 1:-1, :] * (ti + Sa_time_down[t, 1:-1, :]) * (
                                                      Sa_track_down[t, 1:-1, :] / W_down[t, 1:-1, :])
                                              + Fa_downward[t, 1:-1, :] * (ti + Sa_time_top[t, 1:-1, :]) * (
                                                      Sa_track_top[t, 1:-1, :] / W_top[t, 1:-1, :])
                                              - Fa_upward[t, 1:-1, :] * (ti + Sa_time_down[t, 1:-1, :]) * (
                                                      Sa_track_down[t, 1:-1, :] / W_down[t, 1:-1, :])
        ...
        Sa_time_after_Fa_top[0, 1:-1, :] = ((Sa_track_top[t, 1:-1, :] * (ti + Sa_time_top[t, 1:-1, :])
                                             - Fa_E_top_WE[t, 1:-1, :] * (ti + Sa_time_top[t, 1:-1, :]) * (
                                                     Sa_track_top[t, 1:-1, :] / W_top[t, 1:-1, :])
                                             + Fa_E_top_EW[t, 1:-1, :] * (ti + Sa_time_E_top[0, 1:-1, :]) * (
                                                     Sa_track_E_top[0, 1:-1, :] / Sa_E_top[0, 1:-1, :])
                                             + Fa_W_top_WE[t, 1:-1, :] * (ti + Sa_time_W_top[0, 1:-1, :]) * (
                                                     Sa_track_W_top[0, 1:-1, :] / Sa_W_top[0, 1:-1, :])
                                             - Fa_W_top_EW[t, 1:-1, :] * (ti + Sa_time_top[t, 1:-1, :]) * (
                                                     Sa_track_top[t, 1:-1, :] / W_top[t, 1:-1, :])
                                             - Fa_N_top_SN[t, 1:-1, :] * (ti + Sa_time_top[t, 1:-1, :]) * (
                                                     Sa_track_top[t, 1:-1, :] / W_top[t, 1:-1, :])
                                             + Fa_N_top_NS[t, 1:-1, :] * (ti + Sa_time_N_top[0, 1:-1, :]) * (
                                                     Sa_track_N_top[0, 1:-1, :] / Sa_N_top[0, 1:-1, :])
                                             + Fa_S_top_SN[t, 1:-1, :] * (ti + Sa_time_S_top[0, 1:-1, :]) * (
                                                     Sa_track_S_top[0, 1:-1, :] / Sa_S_top[0, 1:-1, :])
                                             - Fa_S_top_NS[t, 1:-1, :] * (ti + Sa_time_top[t, 1:-1, :]) * (
                                                     Sa_track_top[t, 1:-1, :] / W_top[t, 1:-1, :])
                                             - Fa_downward[t, 1:-1, :] * (ti + Sa_time_top[t, 1:-1, :]) * (
                                                     Sa_track_top[t, 1:-1, :] / W_top[t, 1:-1, :])
                                             + Fa_upward[t, 1:-1, :] * (ti + Sa_time_down[t, 1:-1, :]) * (
        ...
        Sa_time_after_Fa_P_E_down[0, 1:-1, :] = ((Sa_track_after_Fa_down[0, 1:-1, :] * Sa_time_after_Fa_down[0, 1:-1, :]
                                                  - P[t, 1:-1, :] * (ti + Sa_time_down[t, 1:-1, :]) * (
                                                          Sa_track_down[t, 1:-1, :] / W[t, 1:-1, :])
                                                  + E_region[t, 1:-1, :] * ti / 2
...
def create_empty_array(count_time, divt, latitude, longitude, yearpart, years):
    ...
    if yearpart[0] == 0:
        sio.savemat(datapathea[0], {'Sa_track_top': Sa_track_top, 'Sa_track_down': Sa_track_down, },
                    do_compression=True)
...
for yearnumber in years:
    ...
    if (yearpart[-1] == 30) & (calendar.isleap(yearnumber) == 0):
    ...
    for a in thisyearpart:
        ...
        if a == 0:  # a == 1 January
            previous_data_to_load = (str(yearnumber - 1) + '-' + str(30))
            # previous_data_to_load = (str(yearnumber - 1) + '-' + str(91))
        else:  # a != 1 January
        ...
        if timetracking == 0:
            Sa_track_top, Sa_track_down, north_loss, south_loss, down_to_top, top_to_down, water_lost = get_Sa_track_forward(
                latitude, longitude, count_time, divt, Kvf, Region, Fa_E_top, Fa_N_top, Fa_E_down, Fa_N_down,
                Fa_Vert, E, P, W_top, W_down, Sa_track_top_last, Sa_track_down_last)
        elif timetracking == 1:
            ...
            Sa_time_top, Sa_time_down, Sa_track_top, Sa_track_down, north_loss, south_loss, down_to_top, top_to_down, water_lost = get_Sa_track_forward_TIME(
                latitude, longitude, count_time, divt, timestep, Kvf, Region, Fa_E_top, Fa_N_top, Fa_E_down, Fa_N_down,
                Fa_Vert, E, P,
                W_top, W_down, Sa_track_top_last, Sa_track_down_last, Sa_time_top_last, Sa_time_down_last)

        # save this data 
        sio.savemat(datapath[3],
                    {'Sa_track_top': Sa_track_top, 'Sa_track_down': Sa_track_down, 'north_loss': north_loss,
                     'south_loss': south_loss, 'down_to_top': down_to_top, 'top_to_down': top_to_down,
                     'water_lost': water_lost}, do_compression=True)
        if timetracking == 1:
            sio.savemat(datapath[4], {'Sa_time_top': Sa_time_top, 'Sa_time_down': Sa_time_down}, do_compression=True)

        end = timer()
        print('Runtime Sa_track for day ' + str(a + 1) + ' in year ' + str(yearnumber) + ' is', (end - start),
              ' seconds.')
