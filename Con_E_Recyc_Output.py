...
years = np.arange(2012, 2017)  # Note that this can be forward as tracking was already finished, whereas the masterscript must be in backward order
...
output_folder = r'F:\WAM2\WAM2layersPython\ERA-Interim\DATA\output'
...
def data_path(y, a, years, timetracking):
    ...
    save_path = os.path.join(output_folder,
                             'E_track_continental_full' + str(years[0]) + '-' + str(years[-1]) + '-timetracking' + str(
                                 timetracking) + '.mat')

    save_path_daily = os.path.join(output_folder, 'E_track_continental_daily_full' + str(y) + '-timetracking' + str(
        timetracking) + '.mat')
...
E_track_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
...
if timetracking == 1:
    ...
    E_time_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
...
for i in range(len(years)):
    ...
    E_track_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    ...
    if timetracking == 1:
        ...
        E_time_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    ...
    if daily == 1:
        ...
        sio.savemat(datapath[4],
                    {'E_per_day': E_per_day, 'E_track_per_day': E_track_per_day, 'P_per_day': P_per_day,
                     ...
                     'W_down_p34y12`er_day': W_down_per_day, 'W_top_per_day': W_top_per_day,
    ...
    for m in range(1):
        ...
        if timetracking == 1:
            Sa_time_down_per_year_per_month[y - startyear, m, :, :] = (
                np.squeeze(np.mean(Sa_time_down_per_day[days, :, :], axis=0)))
            Sa_time_top_per_year_per_month[y - startyear, m, :, :] = (
                np.squeeze(np.mean(Sa_time_top_per_day[days, :, :], axis=0)))
            E_time_per_year_per_month[y - startyear, m, :, :] = (np.squeeze(np.sum(E_time_per_day[days, :, :]
                                                                                   * E_track_per_day[days, :, :],
                                                                                   axis=0)) / np.squeeze(
                E_track_per_year_per_month[y - startyear, m, :, :]))
...
sio.savemat(datapath[3],
            {'E_per_year_per_month': E_per_year_per_month, 'E_track_per_year_per_month': E_track_per_year_per_month,
