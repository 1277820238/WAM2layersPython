...
yearpart = np.arange(0, 30)  # for a full (leap)year fill in 0:366

# years = np.arange(1950,1951) #fill in the years
# yearpart = np.arange(0,30) # for a full (leap)year fill in 0:366
...
startyear = years[0]
Fa_E_down_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
Fa_E_top_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
Fa_N_down_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
Fa_N_top_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
Fa_Vert_per_year_per_month = np.zeros((len(years), 1, len(latitude), len(longitude)))
...
for y in years:
    ...
    final_time = 30
    #
    Fa_E_down_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    Fa_E_top_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    Fa_N_down_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    Fa_N_top_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    Fa_Vert_per_day = np.zeros((31 + ly, len(latitude), len(longitude)))
    # final_time = 90

    # Fa_E_down_per_day = np.zeros((365, len(latitude), len(longitude)))
    # Fa_E_top_per_day = np.zeros((365, len(latitude), len(longitude)))
    # Fa_N_down_per_day = np.zeros((365, len(latitude), len(longitude)))
    # Fa_N_top_per_day = np.zeros((365, len(latitude), len(longitude)))
    # Fa_Vert_per_day = np.zeros((365, len(latitude), len(longitude)))
    ...
    for m in range(1):
        ...
        Fa_E_down_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.sum(Fa_E_down_per_day[days, :, :], axis=0)))
        Fa_E_top_per_year_per_month[y - startyear, m, :, :] = (np.squeeze(np.sum(Fa_E_top_per_day[days, :, :], axis=0)))
        Fa_N_down_per_year_per_month[y - startyear, m, :, :] = (
            np.squeeze(np.sum(Fa_N_down_per_day[days, :, :], axis=0)))
