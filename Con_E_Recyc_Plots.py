...
# latnrs = np.arange(47,95)
# lonnrs = np.arange(98,180)
...
def data_path(years, timetracking):
    load_data_pypm = os.path.join(output_folder, 'E_track_continental_full' + str(years[0]) + '-' + str(
...
d = nc.Dataset('2012-2016-1-E.nc', 'w')
...
d.createVariable('eps_c_landtotal', 'f', ('latitude', 'longitude'))

y_label = pd.DataFrame(y)

d.variables['longitude'][:] = x[0]
d.variables['latitude'][:] = y_label[0]
d.variables['eps_c_landtotal'][:] = eps_c_landtotal

d.close

shownctest = nc.Dataset(r'2012-2016-1-E', 'r')
print(shownctest.variables)
...
if timetracking == 1:
    ...
    E_c_time_average = (
                (np.squeeze(np.sum(np.sum(E_time_per_year_per_month * E_track_per_year_per_month, axis=0), axis=0))
