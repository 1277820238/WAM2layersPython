...
def getconstants(latnrs, lonnrs, lake_mask,
                 invariant_data):  # def getconstants in Python is the same as function in MATLAB.
    ...
    # lsm = np.squeeze(Dataset(invariant_data, mode='r').variables['lsm'][0,0:241,0:480])[::-1, ::-1]
    ...
    lsm[lsm < landfrc] = 0  # the northern boundary is always oceanic = 0
    lsm[lsm >0] = 1  # the southern boundary is always oceanic = 0
    ...
    A_gridcell[:, 0] = (np.pi / 180.0) * Erad ** 2 * abs(
        np.sin(lat_s_bound * np.pi / 180.0) - np.sin(lat_n_bound * np.pi / 180.0)) * gridcell
    ...
    L_N_gridcell = gridcell * np.cos(
        (latitude + gridcell / 2.0) * np.pi / 180.0) * dg  # [m] length northern boundary of a cell
    L_S_gridcell = gridcell * np.cos(
        (latitude - gridcell / 2.0) * np.pi / 180.0) * dg  # [m] length southern boundary of a cell
